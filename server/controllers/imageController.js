...
// Helper function to generate 360° image with Gemini AI (Free)
const generate360ImageWithGemini = async (prompt, folder = "scenes") => {
  try {
    const model = genAI.getGenerativeModel({
      model: "gemini-3-pro-image-preview",
    });

    console.log("Generating 360° panoramic image with Gemini AI (Free)...");
    console.log("Prompt:", prompt);

    // Enhanced prompt specifically for 360° panoramic images
    const enhancedPrompt = `${prompt}. Create a 360-degree panoramic image in equirectangular projection format. The image should be a seamless spherical panorama that wraps around completely, showing a full 360-degree view in all directions. The aspect ratio should be 2:1 (width:height) for proper panoramic display. The image should be continuous and seamless when viewed in a 360-degree viewer.`;
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: `Generate a 360-degree panoramic image: ${enhancedPrompt}`,
            },
          ],
        },
      ],
      ...
      if (candidate.content && candidate.content.parts) {
        for (const part of candidate.content.parts) {
          if (part.inlineData && part.inlineData.data) {
            ...
            console.log(
              "Uploading 360° panoramic image to Firebase Storage..."
            );
    ...
    if (!imageUrl) {
      console.log("No image generated by Gemini for 360°, using placeholder");
      imageUrl = `https://via.placeholder.com/2048x1024/10b981/ffffff?text=${encodeURIComponent(
        prompt.substring(0, 30) + " 360°"
      )}`;
    }

    return imageUrl;
  } catch (error) {
    console.error("Error in generate360ImageWithGemini:", error);
    return `https://via.placeholder.com/2048x1024/10b981/ffffff?text=360+Generation+Failed`;
  }
};
...
const generateScene = async (req, res) => {
  try {
    ...
    // Use Gemini AI for both normal and 360° images (Free)
    if (outputType === "360") {
      // Use Gemini with enhanced prompt for 360° panoramic images
      imageUrl = await generate360ImageWithGemini(enhancedPrompt, "scenes");
    } else {
      // Use Gemini for normal images
